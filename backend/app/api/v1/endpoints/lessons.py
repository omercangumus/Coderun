...
@router.get("/module/{module_id}", response_model=list[LessonWithProgressResponse])
async def list_lessons_by_module(
    ...
    """
    return await lesson_service.get_lessons_by_module(
        module_id, current_user.id, lesson_repo, progress_repo
    )
